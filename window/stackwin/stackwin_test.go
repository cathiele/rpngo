...
func TestSetGetProp(t *testing.T) {
	data := []struct {
		...
		{
			name:        "round",
			set:         rpn.Frame{Type: rpn.INTEGER_FRAME, Int: 2},
			wantDefault: "-1d",
		},
		{
			name:       "round",
			set:        rpn.Frame{Type: rpn.STRING_FRAME, Str: "hello"},
			wantSetErr: rpn.ErrIllegalValue,
		},
		{
			name: "round",
			set:  rpn.Frame{Type: rpn.INTEGER_FRAME, Int: -1},
		},
		{
			name:       "round",
			set:        rpn.Frame{Type: rpn.STRING_FRAME, Int: -2},
			wantSetErr: rpn.ErrIllegalValue,
		},
		{
			name:       "round",
			set:        rpn.Frame{Type: rpn.STRING_FRAME, Int: 11},
			wantSetErr: rpn.ErrIllegalValue,
		},
		{
			name:       "foo",
...
func TestRoundedString(t *testing.T) {
	data := []struct {
		...
		{
			round: 2,
			val:   rpn.Frame{Type: rpn.STRING_FRAME, Str: "hello"},
			want:  "\"hello\"",
		},
		{
			round: 2,
			val:   rpn.Frame{Type: rpn.INTEGER_FRAME, Int: 2},
			want:  "2d",
		},
		{
			round: 0,
			val:   rpn.Frame{Type: rpn.COMPLEX_FRAME, Complex: complex(pi, 0)},
			want:  "3",
		},
		{
			round: 1,
		...
		{
			round: 0,
			val:   rpn.Frame{Type: rpn.COMPLEX_FRAME, Complex: complex(0, pi)},
			want:  "3i",
		},
		{
			round: 1,
		...
		{
			round: 0,
			val:   rpn.Frame{Type: rpn.COMPLEX_FRAME, Complex: complex(pi, pi)},
			want:  "3+3i",
		},
		{
			round: 1,
